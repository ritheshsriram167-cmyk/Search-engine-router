...
function StatLedger({ providers, brainKeys, stats }) {
  ...
  const failed24h = providers.reduce((s, p) => s + (p.keys?.filter(k => k.status !== 'active').length || 0), 0)
    + brainKeys.filter(k => k.status !== 'active').length;
  const brainState = providerStatus(brainKeys);

  const statItems = [
    { label: 'Requests today', value: stats?.requestsToday || '1,204' },
    ...
    <div style={{
      ...
      {statItems.map((s, i) => (
        <div key={s.label} style={{
          flex: '1 1 140px', padding: '18px 22px', borderRight: i < statItems.length - 1 ? `1px solid ${C.borderSoft}` : 'none',
        }}>
          <div style={{ fontFamily: fontMono, fontSize: 24, fontWeight: 500, color: s.color || C.text, marginBottom: 4, textTransform: s.color ? 'capitalize' : 'none' }}>{s.value}</div>
          <div style={{ fontFamily: fontHead, fontSize: 12.5, color: C.textFaint }}>{s.label}</div>
        </div>
      ))}
    </div>
  );
...
function RequestChart({ chartData }) {
  ...
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: '20px 22px', marginBottom: 24 }}>
      <div style={{ fontFamily: fontHead, fontSize: 13, fontWeight: 600, color: C.textSoft, marginBottom: 16 }}>Requests, last 7 days</div>
      <div style={{ height: 180 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData || defaultData} margin={{ top: 4, right: 4, bottom: 0, left: -18 }}>
            <defs>
            ...
            <YAxis tick={{ fill: C.textFaint, fontFamily: fontMono, fontSize: 11 }} axisLine={false} tickLine={false} />
            <Tooltip
...
function ProviderBreakdown({ providers, providerStats }) {
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: '20px 22px', marginBottom: 24, overflowX: 'auto' }}>
      <div style={{ fontFamily: fontHead, fontSize: 13, fontWeight: 600, color: C.textSoft, marginBottom: 14 }}>Provider breakdown</div>
      ...
      {providers.map(p => {
        const stats = (providerStats && providerStats[p.id]) || { routed: 280, successRate: 98 };
  ...
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: '20px 22px', marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        ...
        <div style={{ fontFamily: fontHead, fontSize: 13, fontWeight: 600, color: C.text }}>Live Search Stream Tester (Zero-Cost SSE)</div>
      </div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
        <input
          ...
          placeholder="Enter a test query (e.g. 'breaking tech news' or 'quantum computing database')"
          style={{
      ...
      {streamLog.length > 0 && (
        <div style={{ background: C.bg, border: `1px solid ${C.borderSoft}`, borderRadius: 6, padding: '12px 14px', maxHeight: 220, overflowY: 'auto' }}>
...
function ActivityFeed({ log }) {
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: '20px 22px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        <div style={{ fontFamily: fontHead, fontSize: 13, fontWeight: 600, color: C.textSoft }}>Activity</div>
...
function DashboardTab({ providers, brainKeys, stats, apiKey }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '36px 44px' }}>
        ...
        {active === 'dashboard' && <DashboardTab providers={providers} brainKeys={brainKeys} stats={stats} apiKey={apiKey} />}
      </div>
    </div>
  );
}
